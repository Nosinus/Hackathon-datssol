...
def _load_dotenv(path: str | Path, *, override_existing: bool = False) -> None:
    env_path = Path(path)
    ...
    for raw_line in env_path.read_text(encoding="utf-8").splitlines():
        ...
        if not env_key:
            continue
        if override_existing or env_key not in os.environ:
            os.environ[env_key] = env_value


def load_from_env(*, env_file: str | Path | None = Path(".env")) -> FullSettings:

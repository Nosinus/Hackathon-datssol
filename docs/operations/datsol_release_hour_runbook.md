...
1. Save official files under `docs/input/datssol_imports/` with explicit naming:
   - `{tag}_openapi.json` (if OpenAPI exists)
   - `{tag}_event_notes.md` (manual summary)
   - `{tag}_examples.json` (request/response examples)
2. Keep binaries (if any) in `docs/input/archive/binaries/` and add a short note in `docs/input/datssol_imports/README.md`.
